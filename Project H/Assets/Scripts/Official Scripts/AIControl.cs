        ...
        private void Update()
        {
            if (lookingState == false)      //If player not looking, walking
            {
                ...
                if (target != null)
                {
                    ...
                    if (boundCheck == target.Length) //Looked at all the rooms.
                        inspectionComplete = true;

                    if (Vector3.Distance(gameObject.transform.position, target[targetCounter].GetComponent<Transform>().position) < 1.5 && boundCheck < target.Length)  //If Official is closer than 1.5 to the target and next target is not OutOfBounds
                    {
